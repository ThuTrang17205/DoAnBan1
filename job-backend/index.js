...
// ===================== JOBS API =====================
// ⭐ Mapping slug to Vietnamese category name
...
app.get("/api/jobs/category/:category", async (req, res) => {
  try {
    ...
    // Convert slug to Vietnamese category name
    const categoryName = categorySlugMap[category] || decodeURIComponent(category);
    
    console.log("🔍 Searching for category:", categoryName);
    
    const result = await pool.query(
      "SELECT * FROM raw_jobs WHERE category = $1 ORDER BY crawled_at DESC",
      [categoryName]
    );
    
    console.log(`✅ Found ${result.rows.length} jobs for category: ${categoryName}`);
    res.json(result.rows);
  } catch (err) {
    console.error("❌ Lỗi khi lấy jobs theo category:", err.message);
...
app.get("/api/jobs", async (req, res) => {
  try {
    const result = await pool.query("SELECT * FROM raw_jobs ORDER BY id DESC LIMIT 100");
    res.json(result.rows);
  } catch (err) {
    console.error("❌ Lỗi khi lấy danh sách jobs:", err.message);
...
app.get("/api/jobs/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query("SELECT * FROM raw_jobs WHERE id = $1", [id]);
